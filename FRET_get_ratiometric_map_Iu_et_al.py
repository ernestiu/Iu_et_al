...
import numpy as np
from skimage import io, filters, img_as_ubyte, morphology, measure, exposure
import matplotlib.pyplot as plt
from k_means_segmentation import k_means_seg, binary_operation, remove_top_percentile_pix
from drift_correction import multi_drift_corr, remove_zero_padding, optical_flow_registration, pystackreg_registration, drift_corr, sub_pixel_registration
...
bleedthru_correction = True
yfp_co, cfp_co = 0.05228, 0.3072 # bleedthrough coefficients 
...
clusters = 5 # define the number of clusters used for k means segmentation - higher means a lower threshold
sig_value = 0.4 # 0.4 or 1 # define the sigma value for gaussian blur
smallest_cell_size = 35000 # 6000 for binned images #49000 for regular images # objects smaller than this number (pixel) will be filtered out
fill_holes_on_masks = True # fill holes after segmentation
hole_area = 1500 # only holes smaller than this threshold will be filled
remove_high_intensity_pixels = True # only enable when handling FRET images
...
read_img = 1 # 1 = nd2, 2 = lif, 3 = tif

image_path = '/image_file.nd2'
...
d_channel_idx = 0 # denominator
n_channel_idx = 1 # numerator
segment_channel_idx = 2
...
########
    ...
    if subpixel_registration:
        ...
        for n in range(den.shape[0]):
            ...
            num[n,:,:] = pystackreg_registration(segment_channel[n,:,:], num[n,:,:])
            den[n,:,:] = pystackreg_registration(segment_channel[n,:,:], den[n,:,:])
...
######## 
    ...
    if beads_registration:
        ...
        beads_img = io.imread('/Users/ernestiu/Library/Group Containers/G69SCX94XU.duck/Library/Application Support/duck/Volumes/Images Server/2022-10-26/Cdc42/bead image.tif')
 
        _, num_xoff, num_yoff = optical_flow_registration(beads_img[:,:,2], beads_img[:,:,1])
        print("Numerator channel shifted by: ", round(num_xoff, 2), round(num_yoff, 2))
        
        _, segment_xoff, segment_yoff = optical_flow_registration(beads_img[:,:,2], beads_img[:,:,0])
        print("Denominator channel shifted by: ", round(segment_xoff, 2), round(segment_yoff, 2))       

        for n in range(den.shape[0]):
            
            num[n,:,:] = ndi.shift(num[n,:,:], shift=(num_xoff, num_yoff), mode='constant',  cval=np.median(num[n,:,:]))
            
            segment_channel[n,:,:] = ndi.shift(segment_channel[n,:,:], shift=(segment_xoff, segment_yoff), mode='constant',  cval=np.median(segment_channel[n,:,:]))            
...
########
    ...
    if background_subtraction:
        ...
        for n in range(den.shape[0]):
            ...
            den[n,:,:], den_bg_value = bg_subtraction(den[n,:,:], show_image = n==0, bg_value = den_bg_int)
            all_den_bg_value.append(den_bg_value)
            num[n,:,:], num_bg_value = bg_subtraction(num[n,:,:], show_image = n==0, bg_value = num_bg_int) 
            all_num_bg_value.append(num_bg_value)
            segment_channel[n,:,:], segment_bg_value = bg_subtraction(segment_channel[n,:,:], show_image = n==0, bg_value = seg_bg_int)
            all_segment_channel_value.append(segment_bg_value)

         
        if den.shape[0] > 1: # time-series images 
...
#########
    ...
    if binning:
        ...
        binned_num = np.empty((num.shape[0], 
                               int(np.round(num.shape[1]/bin_size, decimals=0)), 
                              int(np.round(num.shape[2]/bin_size, decimals=0))), 
                              dtype=num.dtype)
...
#########  
    ...
    if ML_segmentation:   # ML segmentation
        ...
        machine_learning_module_path = '//Machine learning codes/'
    ...
    for jj in range(segment_channel.shape[0]):
        ...
        if jj == 0: # display the image for the first frame
            ...
            custom_cmap = colors.LinearSegmentedColormap.from_list("Parula_LUT", ["#352a87","#1685d4","#66be85","#febd3c","#fafb2a"])
            ...
            if not ML_segmentation:
                ...
                plt.imshow(ratiometric_img_thres[jj,:,:], vmin=mean, vmax=mean+std*3, cmap=custom_cmap)
                plt.axis('off')
                plt.show()
            ...
            if ML_segmentation:
                ...
                ax2.imshow(ratiometric_img_thres[jj,:,:], cmap=custom_cmap)#, vmin=100, vmax=500)
    ...
    if save:
        ...
        tifffile.imwrite(final_image_name + '.tif', ratiometric_img_thres.astype(np.uint16), imagej=True, 
